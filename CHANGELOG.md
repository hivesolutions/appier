...
* Refresh of import paths so newly installed optional packages become usable without restarting
* Option to disable automatic installation when importing optional dependencies

### Changed

* Honor an explicitly provided user flag when installing packages
...
* CI build issue preventing new releases

## [1.37.1] - 2026-03-30

### Fixed

* Added `asyncio.create_task()` fallback to `ensure_future()` for Python < 3.7 compatibility in ASGI start response
...
* Avoid re-encoding already encoded byte strings in ASGI response handling
