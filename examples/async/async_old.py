...
class AsyncOldApp(appier.App):
    def __init__(self, *args, **kwargs):
        appier.App.__init__(self, name="async_old", *args, **kwargs)
    ...
    def http(self):
        ...
        for value in appier.header_a():
            yield value
        for value in appier.sleep(delay):
            yield value
    ...
    def handler(self):
        ...
        for value in self.calculator(future, 2, 2):
            yield value
        message += "result: %d\n" % future.result()
        yield message
    ...
    def handler_partial(self):
        ...
        for value in self.calculator(result, 2, 2):
            yield value
        yield "result: %d\n" % result.result()
